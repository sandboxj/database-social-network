...
while ($non_friend = mysqli_fetch_assoc($non_friends_result)) {
	$non_friends[] = $non_friends["UserID"];
	$interests[] = $non_friends["Interest"];
	$birthdates[] = strtotime($non_friends["DateOfBirth"]);
	$locations[] = $non_friends["CurrentLocation"];
}
$do_not_recommend_query = "SELECT * FROM do_not_recommend d
						   WHERE d.UserID = '{$_SESSION["User1ID"]}";
...
for ($non_friend = 0; $non_friend < count($non_friends); $non_friend++) {
	...
	$total_mutual_friends = $total_mutual_friends + $mutual_friend_count;
	$mutual_friends[] = $mutual_friend_count;
...
for ($non_friend = 0; $non_friend < count($non_friends); $non_friend++) {
	...
	if ($total_mutual_friends == 0) {
        ...
        //print $non_friends[$non_friend] . " x " . 0.7*$mutual_friends[$non_total]/$count_total . " x " . 0.1*(1-$distance/$sum_of_distances) . " x " . 0.1*$share_interest . " x " . 0.1*(1-$age_differences[$non_friend]/$sum_of_age_differences) . " x " . $score . " z <br/>";
        $scores[$non_friend] = $score;
	}
}
arsort($scores);
